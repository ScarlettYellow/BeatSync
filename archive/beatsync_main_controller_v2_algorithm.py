...
BeatSync 主控程序
实现分支处理逻辑，支持所有case类型：
- 第一类badcase: 使用beatsync_badcase_fix_trim_v2.py
- 其他case: 使用beatsync_fine_cut_modular.py
"""
...
def detect_badcase_type(dance_video: str, bgm_video: str, sr: int = 22050) -> Tuple[str, float]:
    ...
    try:
        ...
        # 检测badcase类型（与V2版本逻辑一致）
        if T1 > T2:
            gap_duration = T1 - T2
            print(f"检测到第一类badcase: T1({T1:.2f}s) > T2({T2:.2f}s), 需要裁剪dance视频前{gap_duration:.2f}s")
            return "T1_GT_T2", gap_duration
        elif T2 > T1:
            gap_duration = T2 - T1
            print(f"检测到第二类badcase: T2({T2:.2f}s) > T1({T1:.2f}s), 需要裁剪BGM视频前{gap_duration:.2f}s")
...
def process_first_type_badcase(dance_video: str, bgm_video: str, output_video: str) -> bool:
    """处理第一类badcase，使用V2版本"""
    try:
        print("使用第一类badcase处理程序...")
        
        # 调用beatsync_badcase_fix_trim_v2.py
        script_path = os.path.join(os.path.dirname(__file__), "beatsync_badcase_fix_trim_v2.py")
        ...
        if result.returncode == 0:
            print("第一类badcase处理成功")
            return True
        else:
            print(f"第一类badcase处理失败: {result.stderr}")
            return False
            
    except Exception as e:
        print(f"第一类badcase处理异常: {e}")
        return False
...
def process_normal_case(dance_video: str, bgm_video: str, output_video: str) -> bool:
    ...
    try:
        ...
        # 调用beatsync_fine_cut_modular.py
        script_path = os.path.join(os.path.dirname(__file__), "beatsync_fine_cut_modular.py")
        ...
        if result.returncode == 0:
            ...
            print(f"正常case处理失败: {result.stderr}")
            return False
            
    except Exception as e:
        print(f"正常case处理异常: {e}")
        return False

def process_beat_sync(dance_video: str, bgm_video: str, output_video: str) -> bool:
    """主处理函数，实现分支逻辑"""
    print("=" * 60)
    print("BeatSync 主控程序启动")
    print("=" * 60)
    ...
    print(f"输入文件:")
    print(f"  Dance视频: {dance_video}")
    print(f"  BGM视频: {bgm_video}")
    print(f"  输出视频: {output_video}")
    print()
    
    # 步骤1: 检测badcase类型
    print("步骤1: 检测badcase类型...")
    badcase_type, gap_duration = detect_badcase_type(dance_video, bgm_video)
    
    if badcase_type == "ERROR":
        print("badcase检测失败，无法继续处理")
        return False
    
    print()
    
    # 步骤2: 根据badcase类型选择处理路径
    print("步骤2: 选择处理路径...")
    
    if badcase_type == "T1_GT_T2":
        print("检测到第一类badcase，使用V2版本处理...")
        success = process_first_type_badcase(dance_video, bgm_video, output_video)
    elif badcase_type == "T2_GT_T1":
        print("检测到第二类badcase，使用modular版本处理...")
...
def main():
    parser = argparse.ArgumentParser(description="BeatSync 主控程序 - 支持所有case类型")
